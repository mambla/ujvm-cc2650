...
UInt8 ujInitAllClasses(void){
	...
	while(cls) {
	    ujLog("XXXXX class %d (%p), next: %p\n", count, cls, cls->nextClass);
        cls = cls->nextClass;
        count++;
	}
	...
	while(cls){
		...
		if(ret == UJ_ERR_METHOD_NONEXISTENT){
		    ...
		    ujLog("Running class %d clinit\n", count);
			while(ujCanRun()) ujInstr();
			threadH = 0;
		}
		
		cls = cls->nextClass;
		ujLog("Initialized class %d, next: %p\n", count, cls);
		count++;
	}
...
static UInt8 ujNat_RT_consolePut(UjThread* t, _UNUSED_ UjClass* myCls){
    ...
    uart_putchar((char)ujThreadPop(t));
	return UJ_ERR_NONE;
}

static UInt8 ujNat_RT_srand(UjThread* t, _UNUSED_ UjClass* myCls){

    srand((char)ujThreadPop(t));
    return UJ_ERR_NONE;
}

static UInt8 ujNat_RT_rand(UjThread* t, _UNUSED_ UjClass* myCls)
{
    return ujThreadPush(t, rand(), false) ? UJ_ERR_NONE : UJ_ERR_STACK_SPACE;
}
